...
type Persistence struct {
	...
	// If it's set to Delete and the bookkeeper cluster is deleted, the corresponding PVCs will be deleted.
...
func (in *BookkeeperClusterSpec) setDefaults() (changed bool) {
	if in.Image.SetDefaults(defaultImageRepository, defaultImageTag, v1.PullIfNotPresent) {
		changed = true
	}
	if in.Size == 0 {
		changed = true
