...
func updateStatefulsetPvs(ctx reconciler.Context, sts *v1.StatefulSet, cluster *v1alpha1.BookkeeperCluster) error {
		...
		Handler: v12.Handler{
			Exec: &v12.ExecAction{
				Command: []string{"/bin/sh", "-c",
					fmt.Sprintf("netstat -ltn 2> /dev/null | grep %d || ss -ltn 2> /dev/null | grep %d",
						port, port),
				},
			},
		},
