...
DOCKER_IMAGE=monime/bookkeeper-operator:latest
if [[ -n $VERSION ]]; then
  HELM_VERSION="${VERSION//v/}" #helm version must be a SemVer
  DOCKER_IMAGE="monime/bookkeeper-operator:$VERSION"
...
OPERATOR_NAMESPACE=bookkeeper-operator
...
helm template bookkeeper-operator --include-crds --namespace $OPERATOR_NAMESPACE deployments/charts/operator/ >deployments/manifest.yaml
